...
describe('Leaderboard Service', () => {
	...
	it('should get full leaderboard sorted by score', async () => {
		...
		expect(leaderboard[0].leaderboard.l_score!).toBeGreaterThanOrEqual(
			leaderboard[1].leaderboard.l_score!
		);
	});
...
describe('Request Service', () => {
	...
	it('should return request with associated tags and requested languages', async () => {
        ...
        expect(result.kb_size).toBe(512);
	    expect(result.star_size).toBe(1200);

		// Tags
        expect(Array.isArray(result.tags)).toBe(true);
        const tagValues = result.tags.map((t) => t.tag);
        expect(tagValues).toContain('api');
        ...
        //Check Current Languages --need to add support in create request for this
        expect(Array.isArray(result.cur_languages)).toBe(true);
        expect(result.cur_languages.length).toBe(0);
		//console.log(JSON.stringify(result, null, 2));
	});

	it('should return all requests with associated tags and requested languages by user', async () => {
		...
		const r1Tags = r1?.tags.map((t) => t.tag);
    ...
    it('should return all requests with associated tags and requested languages', async () => {
        ...
        const data = result?.data as serviceTypes.RequestWithLanguageAndTags[];
        
        const r1 = data.find((r) => r.r_id === request1.r_id);
        const r2 = data.find((r) => r.r_id === request2.r_id);
    ...
    it('should filter requests by kb_size range', async () => {
        const [request1] = await createRequest({
            requestor_id: userId,
            repo_url: 'https://github.com/example/repo1',
            kb_size: 128,
        ...
        const result = await getAllRequests({ minKb: 50, maxKb: 500 });
    
        expect(result).not.toBeNull();
        expect(result?.data).toHaveLength(1);
        expect(result?.data[0].repo_url).toBe(request1.repo_url);
    });
    ...
    it('should filter requests by tag', async () => {
        ...
        const result = await getAllRequests({ tags: ['auth'] });
    
        expect(result).not.toBeNull();
        expect(result?.data).toHaveLength(1);
        expect(result?.data[0].tags.map(t => t.tag)).toContain('auth');
    });
    
    it('should return correct request with combined filters', async () => {
        const [request1] = await createRequest({
            requestor_id: userId,
            repo_url: 'https://github.com/example/repo1',
            kb_size: 48,
        ...
        expect(result?.data).toHaveLength(1);
        const [r] = result?.data!;
        expect(r.repo_url).toBe('https://github.com/example/repo2');
        expect(r.star_size).toBe(1200);
        expect(result?.data[0].requested_languages.map(l => l.language)).toContain('japanese');
        expect(result?.data[0].tags.map(t => t.tag)).toContain('backend');
    });    

    it('should return paginated results with data', async () => {
        // Add 15 dummy requests
        ...
        const resultPage1 = await getAllRequests({ page: 1, perPage: 10 });
      
        expect(resultPage1).not.toBeNull();
        expect(resultPage1?.page).toBe(1);
        expect(resultPage1?.perPage).toBe(10);
        expect(resultPage1?.totalPages).toBeGreaterThanOrEqual(2);
        expect(resultPage1?.data).toHaveLength(10);
      
        // Request page 2 with remaining 5
        const resultPage2 = await getAllRequests({ page: 2, perPage: 10 });
      
        expect(resultPage2).not.toBeNull();
        expect(resultPage2?.page).toBe(2);
        expect(resultPage2?.data).toHaveLength(5);
      
        // Validate dinstinct results
        const urlsPage1 = resultPage1?.data.map(r => r.repo_url);
        const urlsPage2 = resultPage2?.data.map(r => r.repo_url);
        const intersection = urlsPage1?.filter(url => urlsPage2?.includes(url));
